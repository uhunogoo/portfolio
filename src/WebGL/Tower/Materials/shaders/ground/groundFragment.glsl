...
void main() {
    ...
    float strength = rand(i_st, 20.0);
	vec3 color = vec3( newST, 1.0 );
    float square = 1.0 - step(0.4999, max(abs(newST.x - 0.5), abs(newST.y - 0.5)));
	float noise = iqnoise((st - 0.5) * scale + 0.5, 0.5 , .8);

    float size = mix(0.5, 0.8, strength);
    vec2 rotateST = rotate( (newST - 0.5) * size + 0.5, PI * strength, vec2(0.5) );
	...
	vec4 shadow = texture2D(uShadow, (st - 0.5) * 1.04 + 0.5);
	
    vec3 finalGrassTexture = mix(
        grass.rgb,
        ground.rgb,
        smoothstep(-0.1, 0.9, dot(grass.r - noise, ground.r + noise))
    );
    finalGrassTexture = clamp( finalGrassTexture, 0.0, 1.0 );

	vec3 finalTexture = mix(
		finalGrassTexture * 0.25,
		finalGrassTexture,
		shadow.r
	);
